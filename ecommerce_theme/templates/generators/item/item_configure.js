class ItemConfigure {
						...
						change: () => {
							...
							if (value < numbers[0] || value > numbers[1]) {
								...
								control.set_description(
									__("Value must be between {0} and {1}", [
										numbers[0],
										numbers[1],
									])
								);
	...
	get_html_for_item_found({ filtered_items_count, filtered_items, exact_match, product_info }) {
		...
		const item_found_status =
			exact_match.length === 1
				? `<div class="alert alert-success justify-between items-center flex space-y-1 py-2 text-green-600" role="alert">
				<div><div>
					${one_item}
					${
				...
				<a class="px-2 py-0.5 rounded-md duration-500 text-white bg-gray-400 text-center" href data-action="btn_clear_values" data-item-code="${one_item}">
					${__("X")}
				</a>
			</div>`
				: `<div class="alert alert-warning d-flex justify-content-between align-items-center" role="alert">
					...
					<a href class="px-2 py-0.5 rounded-md duration-500 text-white bg-gray-400 text-center" data-action="btn_clear_values">
						${__("X")}
					</a>
			</div>`;
		/* eslint-disable indent */
...
function set_continue_configuration() {
	...
	if (localStorage.getItem(`configure:${itemCode}`)) {
		$btn_configure.text(__("Add To Cart"));
	} else {
		$btn_configure.text(__("Add To Cart"));
	}
}
