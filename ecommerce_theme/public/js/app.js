...
try {
	function validateForm() {
		...
		xhttp.send(
			"name=" + name + "&email=" + email + "&subject=" + subject + "&comments=" + comments
		);
		return false;
	}
			...
			html += `
				<div class="dropdown-item flex space-x-1">
					...
					<div class="col-9" style="white-space: normal;">
						<a class="hover:text-orange-500 lg:text-sm md:text-sm text-xs font-medium" href="/${res.route}">${
				res.web_item_name
			}</a>
					</div>
				</div>
...
webshop.ProductView = class CustomProductView extends OriginalProductView {
	...
	prepare_product_area_wrapper(view) {
		let class_name =
			view == "list"
				...
				: "grid lg:grid-cols-3 md:grid-cols-2 grid-cols-2 gap-6";
	...
	add_paging_section(settings) {
		...
		if (this.products) {
			...
			paging_html += `
				<button class="py-1 px-2 inline-block font-medium tracking-wide align-middle duration-500 text-base text-center bg-orange-500/10 text-orange-500 rounded-md me-2 mt-2 btn-prev" data-start="${
					start - page_length
				}"
					${prev_disable}>
					${__("Prev")}
			...
			paging_html += `
				<button class="py-1 px-2 inline-block font-medium tracking-wide align-middle duration-500 text-base text-center bg-orange-500/10 text-orange-500 rounded-md me-2 mt-2 btn-next" data-start="${
					start + page_length
				}"
					${next_disable}>
					${__("Next")}
...
webshop.ProductList = class CustomProductList extends OriginalProductList {
	...
	get_wishlist_icon(item) {
        ...
        <li>
			<a href="javascript:void(0)" class="${
				item.wished ? "like-action-wished" : ""
			} size-10 inline-flex items-center justify-center tracking-wide align-middle duration-500 text-center rounded-full bg-white text-slate-900 hover:bg-slate-900 hover:text-white shadow"
	...
	get_title_html(item, title, settings) {
		let title_html = `<a href="/${
			item.route || "#"
		}" class="hover:text-orange-500 text-lg font-medium">${title}</a>`;
		return title_html;
	}
	...
	get_primary_button(item, settings) {
		if (item.has_variants) {
			...
			return `
				<div id="${
					item.name
				}" class="btn-add-to-cart-list py-2 px-5 inline-block font-semibold tracking-wide align-middle duration-500 text-base text-center bg-slate-900 text-white rounded-md
					${item.in_cart ? "hidden" : ""}"
					data-item-code="${item.item_code}"
...
webshop.ProductGrid = class CustomProductGrid extends OriginalProductGrid {
	...
	get_image_html(item, title) {
		...
		if (image) {
			return `
				<a href="/${item.route || "#"}" class="group-hover:scale-110 duration-500">
	...
	get_primary_button(item, settings) {
		if (item.has_variants) {
			return `
					<a style="visibility: visible" href="/${
						item.route || "#"
			...
			return `
					<div id="${
						item.name
					}" class="btn-add-to-cart-list text-sm py-2 px-5 inline-block font-semibold tracking-wide align-middle duration-500 text-base text-center bg-slate-900 text-white w-full rounded-md
						${item.in_cart ? "hidden" : ""}"
						data-item-code="${item.item_code}"
						style="visibility: visible">
						${settings.enable_checkout ? __("Add to Cart") : __("Add to Quote")}
	...
	get_hover_buttons(item, settings) {
		...
		button_html += `<li class="mt-1"><a href="/${
			item.route || "#"
		}" class="size-10 inline-flex items-center justify-center tracking-wide align-middle duration-500 text-center rounded-full bg-white text-slate-900 hover:bg-slate-900 hover:text-white shadow"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-eye size-4"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"></path><circle cx="12" cy="12" r="3"></circle></svg></a></li>`;
		button_html += `</ul>`;
		if (item.discount) {
    ...
    `;
		...
		frappe.call({
			method: "ecommerce_theme.utils.get_product_reviews",
			type: "GET",
			args: {
				web_item: item.name,
			},
	...
	get_title(item, title) {
		let title_html = `
			...
			<a href="/${
				item.route || "#"
			}" class="hover:text-orange-500 lg:text-lg md:text-base text-sm font-medium">
				${title || ""}
			</a>
		`;
		return title_html;
	...
	get_price_html(item) {
		...
		if (item.formatted_mrp) {
			price_html += `
					<del class="text-slate-400 text-xs">${
